...
| # | Question | Impact si non tranchée | Valeur par défaut proposée |
|---|---|---|---|
| D27 | Format papier définitif (A5, A4, 80mm thermique — un seul format ou plusieurs selon le poste de caisse ?), nombre de copies, mentions légales exactes, logo, signatures requises. | Le PDF de reçu ne peut pas être finalisé visuellement. | **Implémenté provisoirement Lot 4 (17 septembre 2026)** : page web imprimable (`/recus/[id]`, format carte ~400px, pensé pour un rendu A5 via l'impression navigateur — "Enregistrer en PDF" fonctionne nativement) plutôt qu'un vrai PDF généré côté serveur — aucune dépendance ajoutée (pas de Puppeteer/PDFKit), cohérent avec le principe de simplicité du projet tant que le format exact (mentions légales, logo, signatures) n'est pas confirmé par l'école. Remplaçable sans migration de données le jour où D27 est tranchée (le rendu est un template, pas une donnée stockée). |
