...
Permissions envisagées (à confirmer au Lot 7) : `TIMETABLE_MANAGE` (retenue en pratique : `PEDAGOGY_MANAGE` couvre la saisie de l'emploi du temps, pas de permission distincte au Lot 8), `TIMETABLE_READ` (créée au Lot 8), `ATTENDANCE_TAKE`, `ATTENDANCE_CORRECT` (créées au Lot 9, avec `ATTENDANCE_READ` pour la lecture et un rôle `SURVEILLANT`), `TEACHER_CHECKIN_VALIDATE`, `MESSAGE_SUPERVISE`, `ANNOUNCEMENT_PUBLISH`, `PARENT_ACCOUNT_MANAGE`. Chaque action reste gardée côté serveur, jamais seulement masquée dans l'interface.
