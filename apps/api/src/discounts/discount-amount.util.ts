 ...
 */
export function computeApprovedDiscountAmount(line: InvoiceLine, discounts: Discount[]): number {
  const total = discounts
    .filter((d) => d.statut === 'APPROUVEE')
    .reduce((sum, d) => {
      const amount = d.type === 'POURCENTAGE' ? Math.round((line.montant * d.valeur) / 100) : d.valeur;
      return sum + amount;
    }, 0);
