...
describe('Comptes parents et portail (e2e, Lot 11)', () => {
  ...
  const activate = (telephone: string, code: string, extra: object = {}) =>
    portalPost('/portal/activate', { telephone, code, motDePasse: 'MotDePasse123', consentement: true, versionPolitique: '2026-09-v4', ...extra });
  ...
  async function activeParent(guardianId: string, telephone: string, motDePasse = 'MotDePasse123') {
    ...
    const res = await portalPost('/portal/activate', { telephone, code: body.code, motDePasse, consentement: true, versionPolitique: '2026-09-v4' }).expect(201);
  ...
  describe('activation par code (D66, D76)', () => {
    ...
    it('active le compte : mot de passe choisi, consentement enregistré, numéro saisi sous une autre forme', async () => {
      ...
      expect(consent[0].version).toBe('2026-09-v4');
      ...
      expect(list.compte).toMatchObject({ statut: 'ACTIF', consentement: { version: '2026-09-v4' } });
    ...
    it('exige le consentement et la version en vigueur de la politique', async () => {
      ...
      expect(info.body.version).toBe('2026-09-v4');
    });
