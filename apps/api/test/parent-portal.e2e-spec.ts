...
describe('Comptes parents et portail (e2e, Lot 11)', () => {
  ...
  async function school() {
    ...
    await post(`/students/${brice.student.id}/guardians`, {
      nom: 'Ndinga',
      prenom: 'Rose',
    ...
    const record = call.eleves.find(
      (e: { studentId: string }) => e.studentId === alice.student.id,
    ).recordId;
    const reason = (
      await post('/absence-reasons', { libelle: 'Maladie' }).expect(201)
    ).body;
    const just = (
      await post(`/attendance/records/${record}/justification`, {
        reasonId: reason.id,
      }).expect(201)
    ).body;
    await patch(`/attendance/justifications/${just.id}`, {
      statut: 'ACCEPTEE',
    }).expect(200);

    // Alice : un paiement partiel.
    const invoice = (
      await get(`/invoices/by-enrollment/${alice.enrollment.id}`).expect(200)
    ).body;
    await post('/payments', {
  ...
  describe('permissions côté école', () => {
    it('le secrétariat gère les comptes, seule la Direction retire un accès', async () => {
      ...
      const link = await prisma.studentGuardian.findFirstOrThrow({
        where: { guardianId: s.moukala.id, studentId: s.alice.student.id },
      });
      await patch(
      ...
      await patch(`/parent-accounts/links/${link.id}/access`, {
        acces: false,
        motif: 'x',
      }).expect(403); // Administrateur : pas la Direction (D67)
      await patch(
        `/parent-accounts/links/${link.id}/access`,
        { acces: false, motif: 'x' },
        dir,
    ...
    it('liste les responsables avec leurs enfants et l’état du compte, et se recherche', async () => {
      const s = await school();
      const all = (await get('/parent-accounts').expect(200)).body as Array<{
        id: string;
        nom: string;
        compte: unknown;
        enfants: unknown[];
      ...
      const found = (await get('/parent-accounts?search=carine').expect(200))
        .body;
      expect(found.map((g: { id: string }) => g.id)).toEqual([s.zola.id]);
      const byPhone = (
        await get('/parent-accounts?search=060000003').expect(200)
      ).body;
  ...
  describe('activation par code (D66, D76)', () => {
    it('génère un code lisible dont seule l’empreinte est conservée, et un nouveau code invalide l’ancien', async () => {
      ...
      await post(
        '/parent-accounts/guardians/inconnu/activation-code',
        {},
      ).expect(404);
    });
    ...
    it('active le compte : mot de passe choisi, consentement enregistré, numéro saisi sous une autre forme', async () => {
      ...
      const list = (await get('/parent-accounts').expect(200)).body.find(
        (g: { id: string }) => g.id === s.moukala.id,
      );
      expect(list.compte).toMatchObject({
        statut: 'ACTIF',
        consentement: { version: '2026-09-v6' },
      });
    ...
    it('exige le consentement et la version en vigueur de la politique', async () => {
      ...
      await activate(PHONE_MOUKALA, code, {
        versionPolitique: 'ancienne',
      }).expect(400);
    ...
    it('un code faux ne dit jamais si le numéro existe, et se brûle après cinq essais', async () => {
      ...
      for (let i = 0; i < 4; i += 1)
        await activate(PHONE_MOUKALA, 'CCCC-DDDD').expect(401);
    ...
    it('un nouveau code réinitialise le mot de passe oublié et ferme les anciennes sessions', async () => {
      const s = await school();
      const first = await activeParent(
        s.moukala.id,
        PHONE_MOUKALA,
        'AncienMotDePasse1',
      );
      ...
      await activate(PHONE_MOUKALA, code, {
        motDePasse: 'NouveauMotDePasse1',
      }).expect(201);
      await portalPost('/portal/login', {
        telephone: PHONE_MOUKALA,
        motDePasse: 'AncienMotDePasse1',
      }).expect(401);
      await portalPost('/portal/login', {
        telephone: PHONE_MOUKALA,
        motDePasse: 'NouveauMotDePasse1',
  ...
  describe('connexion et session', () => {
    it('refuse un mauvais mot de passe sans rien révéler, verrouille après cinq échecs, respecte la désactivation', async () => {
      ...
      const bad = await portalPost('/portal/login', {
        telephone: PHONE_MOUKALA,
        motDePasse: 'faux',
      }).expect(401);
      const unknown = await portalPost('/portal/login', {
      ...
      await portalPost('/portal/login', {
        telephone: PHONE_MOUKALA,
        motDePasse: 'MotDePasse123',
      }).expect(403); // verrouillé

      await prisma.parentAccount.updateMany({
        data: { verrouilleJusqua: null, tentativesEchecsConnexion: 0 },
      });
      const ok = (
        await portalPost('/portal/login', {
          telephone: PHONE_MOUKALA,
      ...
      await post(
        `/parent-accounts/guardians/${s.moukala.id}/reactivate`,
        {},
      ).expect(201);
      await portalPost('/portal/login', {
        telephone: PHONE_MOUKALA,
      ...
      await post(
        `/parent-accounts/guardians/${s.zola.id}/deactivate`,
        {},
      ).expect(404); // pas de compte
    });
    ...
    it('renouvelle la session avec le cookie, la ferme à la déconnexion, et change le mot de passe', async () => {
      ...
      const login = await agent
        .post('/portal/login')
        .send({ telephone: PHONE_MOUKALA, motDePasse: 'MotDePasse123' })
        .expect(201);
      ...
      await patch(
        ...
        { ancienMotDePasse: 'faux', nouveauMotDePasse: 'AutreMotDePasse1' },
      ...
      await patch(
        ...
        {
          ancienMotDePasse: 'MotDePasse123',
          nouveauMotDePasse: 'AutreMotDePasse1',
        },
        login.body.accessToken,
      ).expect(200);
      await portalPost('/portal/login', {
  ...
  describe('isolation (RV08)', () => {
    it('un parent ne voit que ses enfants, jamais ceux d’une autre famille', async () => {
      ...
      expect(
        me.enfants.map((e: { prenom: string }) => e.prenom).sort(),
      ).toEqual(['Alice', 'Brice']);
      expect(me.enfants[0]).toMatchObject({
        classe: 'CM2 A',
        anneeScolaire: '2026-2027',
      });
      const meZola = (await get('/portal/me', zola).expect(200)).body;
      expect(meZola.enfants.map((e: { prenom: string }) => e.prenom)).toEqual([
        'Carine',
      ]);
      ...
      for (const section of ['timetable', 'attendance', 'finance']) {
        await get(
          `/portal/children/${s.carine.student.id}/${section}`,
          moukala,
        ).expect(404);
        await get(
          `/portal/children/${s.alice.student.id}/${section}`,
          zola,
        ).expect(404);
    ...
    it('un jeton du personnel n’ouvre pas le portail, et un jeton de parent n’ouvre aucune route du personnel', async () => {
      ...
      for (const path of [
        '/auth/me',
        '/students',
        `/students/${s.carine.student.id}`,
        '/rooms',
        '/school',
        '/academic-years',
        '/classes',
        '/absence-reasons',
        `/students/${s.carine.student.id}/financial-status`,
        `/payments?studentId=${s.carine.student.id}`,
      ...
      await post(
        '/payments',
        { invoiceLineId: 'x', montant: 1 },
        parent,
      ).expect(401);
      await post(
        '/attendance/calls',
        { entryId: 'x', date: today, absences: [] },
        parent,
      ).expect(401);
    });
    ...
    it('le portail est en lecture seule : aucune route d’écriture (hors mot de passe)', async () => {
      ...
      const parent = await activeParent(s.moukala.id, PHONE_MOUKALA);
      for (const path of [
        `/portal/children/${s.alice.student.id}/attendance`,
        `/portal/children/${s.alice.student.id}/finance`,
        '/portal/me',
      ]) {
    ...
    it('retirer l’accès d’un responsable pour un enfant le masque, sans toucher aux autres responsables ni aux autres enfants', async () => {
      ...
      const link = await prisma.studentGuardian.findFirstOrThrow({
        where: { guardianId: s.moukala.id, studentId: s.brice.student.id },
      });
      ...
      await patch(
        `/parent-accounts/links/${link.id}/access`,
        { acces: false, motif: 'Décision de justice du 12 septembre' },
        dir,
      ).expect(200);

      const me = (await get('/portal/me', moukala).expect(200)).body;
      expect(me.enfants.map((e: { prenom: string }) => e.prenom)).toEqual([
        'Alice',
      ]); // Brice a disparu
      await get(
        `/portal/children/${s.brice.student.id}/attendance`,
        moukala,
      ).expect(404);
      await get(
        `/portal/children/${s.brice.student.id}/finance`,
        moukala,
      ).expect(404);
      await get(
        `/portal/children/${s.alice.student.id}/finance`,
        moukala,
      ).expect(200); // ses autres enfants restent accessibles
      ...
      const listed = (await get('/parent-accounts').expect(200)).body.find(
        (g: { id: string }) => g.id === s.moukala.id,
      );
      expect(
        listed.enfants.find(
          (e: { studentId: string }) => e.studentId === s.brice.student.id,
        ),
      ).toMatchObject({
        accesPortail: false,
        accesMotif: 'Décision de justice du 12 septembre',
      });

      // Rétablissable, avec motif.
      await patch(
        `/parent-accounts/links/${link.id}/access`,
        { acces: true, motif: 'Décision levée' },
        dir,
      ).expect(200);
      await get(
        `/portal/children/${s.brice.student.id}/finance`,
  ...
  describe('ce que voit le parent', () => {
    it('emploi du temps de la classe de l’enfant, sans les motifs internes ni les autres classes', async () => {
      ...
      const entry = await prisma.timetableEntry.findFirstOrThrow({
        where: { classId: s.klassA.id, jourSemaine: weekdayOf(yesterday) },
      });
      const other = (await post('/teachers', { nom: 'Okemba', prenom: 'Jean' }))
        .body;
      await post(`/timetable-entries/${entry.id}/exceptions`, {
        date: yesterday,
        type: 'REMPLACEE',
        motif: 'Deuil dans la famille',
        replacementTeacherId: other.id,
      }).expect(201);

      const week = (
        await get(
          `/portal/children/${s.alice.student.id}/timetable?date=${yesterday}`,
          parent,
        ).expect(200)
      ).body;
      expect(week.classe).toBe('CM2 A');
      // Hier : la seule séance de CM2 A (9h), jamais celle de CM2 B prévue à 8h le même jour.
      const day = week.jours.find(
        (j: { date: string }) => j.date === yesterday,
      );
      ...
      const replaced = all.find(
        (x: { statut: string }) => x.statut === 'REMPLACEE',
      );
      expect(replaced).toMatchObject({
        matiere: 'Mathématiques',
        enseignant: 'Jean Okemba',
      });
      expect(JSON.stringify(week)).not.toContain('Deuil');
      expect(JSON.stringify(week)).not.toContain('motif');
      await get(
        `/portal/children/${s.alice.student.id}/timetable?date=demain`,
        parent,
      ).expect(400);
    });
    ...
    it('absences et retards de l’enfant avec le statut du justificatif, sans données internes', async () => {
      ...
      expect(data.compteurs).toMatchObject({
        absences: 1,
        excusees: 1,
        nonJustifiees: 0,
      });
      ...
      expect(data.lignes[0]).toMatchObject({
        matiere: 'Mathématiques',
        statut: 'ABSENT',
        justificatif: { statut: 'ACCEPTEE', motif: 'Maladie' },
      });
      const raw = JSON.stringify(data);
      for (const internal of [
        'corrections',
        'Appel saisi après coup',
        'Ngoma',
        'recordId',
        'studentId',
      ])
  ...
  describe('journal d’audit', () => {
    it('trace le code, l’activation, la connexion et le retrait d’accès, sans jamais le code ni le mot de passe', async () => {
      ...
      const link = await prisma.studentGuardian.findFirstOrThrow({
        where: { guardianId: s.moukala.id },
      });
      await patch(
        `/parent-accounts/links/${link.id}/access`,
        { acces: false, motif: 'Test' },
        dir,
      ).expect(200);
      await post(
        `/parent-accounts/guardians/${s.moukala.id}/deactivate`,
      ...
      expect(actions).toEqual(
        expect.arrayContaining([
          ...
          'PARENT_ACCOUNT_ACTIVATE',
          'PARENT_LOGIN_SUCCESS',
          'PARENT_LINK_ACCESS_REVOKE',
          'PARENT_ACCOUNT_DEACTIVATE',
        ]),
      );
