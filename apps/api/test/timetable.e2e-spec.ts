...
describe('Emploi du temps (e2e, Lot 8)', () => {
  ...
  async function school() {
    ...
    const year = (
      await post('/academic-years', { libelle: '2026-2027', dateDebut: '2026-09-01', dateFin: '2027-07-15' })
    ).body;
    ...
    const pause = (
      await post('/time-slots', { libelle: 'Récréation', heureDebut: '09:40', heureFin: '10:00', type: 'PAUSE' })
    ).body;
    ...
    return { year, level, classA, classB, h1, h2, h3, pause, math, fr, t1, t2, t3, r1, r2 };
  }

  async function draft(yearId: string, extra: object = {}) {
    return (await post('/timetables', { academicYearId: yearId, ...extra }).expect(201)).body;
  }

  const entry = (
    tt: string,
    d: { classA?: boolean; classId: string; subjectId: string; timeSlotId: string; jourSemaine: number; roomId: string },
  ...
  describe('permissions', () => {
    it('sans jeton : 401 ; le Secrétaire-caissier lit mais n’écrit pas ; les brouillons lui sont cachés', async () => {
      ...
      const { user, motDePasse } = await createUserWithRole(prisma, sch.id, 'SECRETAIRE_CAISSIER');
      const t = await login(user.email, motDePasse);

      await post('/timetables', { academicYearId: s.year.id }, t).expect(403);
      await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-09-07' }, t).expect(403);
  ...
  describe('séances et conflits (RV02)', () => {
    it('crée une séance : enseignant déduit de l’affectation, heures du créneau', async () => {
      ...
      const res = await entry(tt.id, {
        classId: s.classA.id, subjectId: s.math.id, timeSlotId: s.h1.id, jourSemaine: 1, roomId: s.r1.id,
      }).expect(201);
    ...
    it('refuse un conflit de classe, d’enseignant et de salle, en nommant la séance en cause', async () => {
      ...
      const classe = await entry(tt.id, {
        classId: s.classA.id, subjectId: s.fr.id, timeSlotId: s.h1.id, jourSemaine: 1, roomId: s.r2.id,
      }).expect(409);
      ...
      const teacher = await entry(tt.id, {
        classId: s.classB.id, subjectId: s.math.id, timeSlotId: s.h1.id, jourSemaine: 1, roomId: s.r2.id,
      }).expect(409);
      expect(teacher.body.message).toContain('Paul Ngoma');

      // Même salle, autre classe, autre enseignant.
      const room = await entry(tt.id, {
        classId: s.classB.id, subjectId: s.fr.id, timeSlotId: s.h1.id, jourSemaine: 1, roomId: s.r1.id,
      }).expect(409);
    ...
    it('autorise deux séances qui s’enchaînent, ou le même créneau un autre jour', async () => {
      const s = await school();
      const tt = await draft(s.year.id);
      const base = { classId: s.classA.id, subjectId: s.math.id, roomId: s.r1.id };
      await entry(tt.id, { ...base, timeSlotId: s.h1.id, jourSemaine: 1 }).expect(201);
      await entry(tt.id, { ...base, timeSlotId: s.h2.id, jourSemaine: 1 }).expect(201);
      await entry(tt.id, { ...base, timeSlotId: s.h1.id, jourSemaine: 2 }).expect(201);
    });

    it('vérifie l’affectation, le créneau (cours, grille), le jour de classe, la salle et l’enseignant actif', async () => {
      const s = await school();
      const tt = await draft(s.year.id);
      const ok = { classId: s.classA.id, subjectId: s.math.id, timeSlotId: s.h1.id, jourSemaine: 1, roomId: s.r1.id };

      const other = (await post('/subjects', { code: 'ART', nom: 'Dessin' })).body;
    ...
    it('modifie et supprime une séance de brouillon, avec re-contrôle des conflits', async () => {
      ...
      await entry(tt.id, {
        classId: s.classB.id, subjectId: s.fr.id, timeSlotId: s.h2.id, jourSemaine: 1, roomId: s.r2.id,
      }).expect(201);

      const moved = await patch(`/timetable-entries/${a.id}`, { timeSlotId: s.h3.id }).expect(200);
      expect(moved.body.heureDebut).toBe('10:00');
      await patch(`/timetable-entries/${a.id}`, { roomId: s.r2.id, timeSlotId: s.h2.id }).expect(409);
  ...
  describe('versions et publication (RV03)', () => {
    ...
    it('refuse de publier un emploi du temps vide ou hors de l’année', async () => {
      const s = await school();
      const tt = await draft(s.year.id);
      await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-09-07' }).expect(422);

      await entry(tt.id, {
        classId: s.classA.id, subjectId: s.math.id, timeSlotId: s.h1.id, jourSemaine: 1, roomId: s.r1.id,
      }).expect(201);
      await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-08-01' }).expect(422);
      await post(`/timetables/${tt.id}/publish`, { dateEffet: '2027-09-01' }).expect(422);
      await post(`/timetables/${tt.id}/publish`, { dateEffet: 'demain' }).expect(400);
    });

    it('publie, puis la version est figée', async () => {
      const { s, tt } = await filled();
      const res = await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-09-07' }).expect(201);
      ...
      await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-09-14' }).expect(409);
    });
    ...
    it('bloque la publication quand un conflit existe (créé après coup par une affectation modifiée)', async () => {
      ...
      await entry(tt.id, {
        classId: s.classB.id, subjectId: s.fr.id, timeSlotId: s.h1.id, jourSemaine: 1, roomId: s.r2.id,
      }).expect(201);

      // CM2 B / Français passe à Ngoma, déjà en classe CM2 A au même moment.
      const asg = (await get(`/assignments?classId=${s.classB.id}`).expect(200)).body.find(
        (a: { subjectId: string }) => a.subjectId === s.fr.id,
      );
      ...
      const res = await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-09-07' }).expect(422);
    ...
    it('versionne : nouvelle version copiée de la publiée, archivage de la précédente, un seul brouillon', async () => {
      const { s, tt } = await filled();
      await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-09-07' }).expect(201);
      ...
      expect((await get(`/timetables/${tt.id}/entries`).expect(200)).body).toHaveLength(1);

      // La date d'effet doit suivre celle de la version en vigueur.
      await post(`/timetables/${v2.id}/publish`, { dateEffet: '2026-09-07' }).expect(422);
      await post(`/timetables/${v2.id}/publish`, { dateEffet: '2026-10-05' }).expect(201);

      const list = (await get(`/timetables?academicYearId=${s.year.id}`).expect(200)).body;
      expect(list.map((t: { numero: number; statut: string }) => [t.numero, t.statut])).toEqual([
        [2, 'PUBLIE'],
        [1, 'ARCHIVE'],
      ]);
    });
    ...
    it('permet de repartir d’un emploi du temps vide, et de supprimer un brouillon', async () => {
      ...
      expect((await get(`/timetables/${v2.id}/entries`).expect(200)).body).toHaveLength(0);
    ...
    it('applique la version en vigueur à chaque date (avant, pendant, après la date d’effet)', async () => {
      const { s, tt } = await filled();
      await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-09-14' }).expect(201);
      const v2 = await draft(s.year.id, { vide: true });
      await entry(v2.id, {
        classId: s.classA.id, subjectId: s.fr.id, timeSlotId: s.h2.id, jourSemaine: 1, roomId: s.r2.id,
      }).expect(201);
      await post(`/timetables/${v2.id}/publish`, { dateEffet: '2026-10-05' }).expect(201);

      const day = async (d: string) => (await get(`/timetable/day?date=${d}`).expect(200)).body;
  ...
  describe('changements ponctuels', () => {
    ...
    const exc = (entryId: string, body: object) => post(`/timetable-entries/${entryId}/exceptions`, body);

    it('annule une séance sans modifier la version publiée (l’historique reste intact)', async () => {
      const { s, tt, e1 } = await published();
      await exc(e1.id, { date: '2026-09-14', type: 'ANNULEE', motif: 'Enseignant absent' }).expect(201);

      const day = (await get('/timetable/day?date=2026-09-14').expect(200)).body;
      const cancelled = day.seances.find((x: { entryId: string }) => x.entryId === e1.id);
      ...
      const other = (await get('/timetable/day?date=2026-09-21').expect(200)).body;
      expect(other.seances.every((x: { statut: string }) => x.statut === 'NORMALE')).toBe(true);

      // La version publiée n'a pas bougé : mêmes séances, même statut, mêmes enseignants.
      const stored = (await get(`/timetables/${tt.id}/entries`).expect(200)).body;
      expect(stored).toHaveLength(2);
      expect(stored.find((x: { id: string }) => x.id === e1.id).teacherId).toBe(s.t1.id);
      expect((await get(`/timetables/${tt.id}`).expect(200)).body.statut).toBe('PUBLIE');
    });
    ...
    it('remplace l’enseignant, avec contrôle de disponibilité du remplaçant', async () => {
      ...
      const busy = await exc(e1.id, {
        date: '2026-09-14', type: 'REMPLACEE', motif: 'Absence', replacementTeacherId: s.t2.id,
      }).expect(409);
      expect(busy.body.message).toContain('Claire Mabiala');
      await exc(e1.id, { date: '2026-09-14', type: 'REMPLACEE', motif: 'Absence' }).expect(422);
      await exc(e1.id, {
        date: '2026-09-14', type: 'REMPLACEE', motif: 'Absence', replacementTeacherId: s.t1.id,
      }).expect(422);

      await exc(e1.id, {
        date: '2026-09-14', type: 'REMPLACEE', motif: 'Absence', replacementTeacherId: s.t3.id,
      }).expect(201);
      const day = (await get(`/timetable/day?date=2026-09-14&teacherId=${s.t3.id}`).expect(200)).body;
    ...
    it('change de salle, avec contrôle d’occupation, et une séance annulée libère sa ressource', async () => {
      ...
      const busy = await exc(e1.id, {
        date: '2026-09-14', type: 'SALLE_MODIFIEE', motif: 'Travaux', roomId: s.r2.id,
      }).expect(409);
      expect(busy.body.message).toContain('Salle 2');
      await exc(e1.id, { date: '2026-09-14', type: 'SALLE_MODIFIEE', motif: 'Travaux' }).expect(422);
      await exc(e1.id, { date: '2026-09-14', type: 'SALLE_MODIFIEE', motif: 'Travaux', roomId: s.r1.id }).expect(422);

      // Une fois la séance de CM2 B annulée, sa salle est libre.
      await exc(e2.id, { date: '2026-09-14', type: 'ANNULEE', motif: 'Sortie' }).expect(201);
      await exc(e1.id, {
        date: '2026-09-14', type: 'SALLE_MODIFIEE', motif: 'Travaux', roomId: s.r2.id,
      }).expect(201);
      const day = (await get(`/timetable/day?date=2026-09-14&roomId=${s.r2.id}`).expect(200)).body;
      expect(day.seances.map((x: { className: string }) => x.className)).toContain('CM2 A');
    });

    it('applique les règles de date, de doublon et de statut', async () => {
      const { s, tt, e1 } = await published();
      await exc(e1.id, { date: '2026-09-15', type: 'ANNULEE', motif: 'x' }).expect(422); // pas un lundi
      await exc(e1.id, { date: '2026-08-31', type: 'ANNULEE', motif: 'x' }).expect(422); // avant la date d'effet
      await exc(e1.id, { date: '2026-09-14', type: 'ANNULEE', motif: '' }).expect(400);
      await exc(e1.id, { date: '2026-09-14', type: 'ANNULEE', motif: 'x' }).expect(201);
      await exc(e1.id, { date: '2026-09-14', type: 'ANNULEE', motif: 'x' }).expect(409); // doublon

      // Un jour sans classe (calendrier) n'a pas de séance à modifier.
      await post('/calendar-events', {
        academicYearId: s.year.id, type: 'FERIE', libelle: 'Fête', dateDebut: '2026-09-21',
      }).expect(201);
      await exc(e1.id, { date: '2026-09-21', type: 'ANNULEE', motif: 'x' }).expect(422);
      const closed = (await get('/timetable/day?date=2026-09-21').expect(200)).body;
      expect(closed.seances).toEqual([]);
      expect(closed.sansClasse.libelle).toBe('Fête');

      // Pas de changement ponctuel sur un brouillon.
      const v2 = await draft(s.year.id);
      const copied = (await get(`/timetables/${v2.id}/entries`).expect(200)).body[0];
      await exc(copied.id, { date: '2026-09-28', type: 'ANNULEE', motif: 'x' }).expect(409);
    ...
    it('refuse un changement sur une version remplacée à cette date, et se supprime proprement', async () => {
      ...
      await exc(e1.id, { date: '2026-10-12', type: 'ANNULEE', motif: 'x' }).expect(422); // v2 en vigueur
      const created = (await exc(e1.id, { date: '2026-09-28', type: 'ANNULEE', motif: 'x' }).expect(201)).body;

      const list = (await get('/timetable-exceptions?from=2026-09-01&to=2026-09-30').expect(200)).body;
      expect(list).toHaveLength(1);
      await del(`/timetable-exceptions/${created.id}`).expect(200);
      expect((await get('/timetable-exceptions').expect(200)).body).toHaveLength(0);
  ...
  describe('vues (classe, enseignant, salle) et semaine', () => {
    it('filtre la semaine par classe, par enseignant et par salle', async () => {
      ...
      await entry(tt.id, {
        classId: s.classB.id, subjectId: s.fr.id, timeSlotId: s.h2.id, jourSemaine: 3, roomId: s.r2.id,
      }).expect(201);
      await post(`/timetables/${tt.id}/publish`, { dateEffet: '2026-09-07' }).expect(201);

      const week = async (q: string) => (await get(`/timetable/week?date=2026-09-09${q}`).expect(200)).body;
      const all = await week('');
      expect(all.debut).toBe('2026-09-07');
      expect(all.fin).toBe('2026-09-13');
      expect(all.jours).toHaveLength(7);
      const count = (w: { jours: { seances: unknown[] }[] }) => w.jours.reduce((n, j) => n + j.seances.length, 0);
  ...
  describe('protections du référentiel (Lot 7)', () => {
    it('refuse de changer ou supprimer un créneau, une salle ou une affectation utilisés', async () => {
      ...
      await patch(`/time-slots/${s.h1.id}`, { libelle: 'Première heure' }).expect(200); // libellé libre
      await del(`/time-slots/${s.h1.id}`).expect(409);
      await del(`/rooms/${s.r1.id}`).expect(409);
      const asg = (await get(`/assignments?classId=${s.classA.id}`).expect(200)).body.find(
        (a: { subjectId: string }) => a.subjectId === s.math.id,
      );
  ...
  describe('année clôturée et audit', () => {
    it('trace la création, la publication et les changements ponctuels dans le journal', async () => {
      ...
      await post(`/timetable-entries/${e.id}/exceptions`, {
        date: '2026-09-14', type: 'ANNULEE', motif: 'x',
      }).expect(201);
