...
describe('Pointage des enseignants (e2e, Lot 10)', () => {
  ...
  async function userToken(roleCode: string, email: string) {
    ...
    const { user, motDePasse } = await createUserWithRole(
      ...
      sch.id,
      roleCode,
      { email, prenom: roleCode, nom: 'Test' },
    );
  ...
  async function school() {
    ...
    for (const sub of [math, fr])
      await put(`/subjects/${sub.id}/levels`, {
    ...
    await post(`/timetables/${tt.id}/publish`, { dateEffet: yesterday }).expect(
      201,
    );
    ...
    await patch(`/teachers/${t2.id}`, {
      userId: p2.user.id,
      modePointage: 'JOURNEE',
    }).expect(200);
    await patch(`/teachers/${t3.id}`, { userId: p3.user.id }).expect(200);

    const codes = (await post('/pointage-codes/generate', {}).expect(201))
      .body as Array<{ type: string; roomId: string | null; token: string }>;
    const tokenOf = (roomId: string | null) =>
      codes.find((c) => c.roomId === roomId)!.token;
    return {
      year,
      classA,
      classB,
      ...
      p3,
      gate: tokenOf(null),
      salle1: tokenOf(room1.id),
      salle2: tokenOf(room2.id),
    };
  }

  const scan = (t: string, code: string, when?: string) =>
    post(
      '/teacher-checkins/scan',
      { code, ...(when ? { scanneLe: when } : {}) },
      t,
    );
  ...
  describe('permissions et QR codes', () => {
    it('chaque rôle n’accède qu’à ce qui le concerne', async () => {
      ...
      await request(app.getHttpServer())
        .get(`/teacher-checkins/day?date=${yesterday}`)
        .expect(401);
      const surv = await userToken('SURVEILLANT', 'surv@test.local');
      const sec = await userToken('SECRETAIRE_CAISSIER', 'sec@test.local');

      await get(`/teacher-checkins/day?date=${yesterday}`, s.p1.token).expect(
        403,
      ); // un enseignant ne lit pas les autres
      await get(`/teacher-checkins/day?date=${yesterday}`, sec.token).expect(
        403,
      );
      await get(`/teacher-checkins/day?date=${yesterday}`, surv.token).expect(
        200,
      );
    ...
    it('génère les QR sans toucher à ceux qui existent, les régénère, et ne les expose jamais ailleurs', async () => {
      const s = await school();
      const again = (await post('/pointage-codes/generate', {}).expect(201))
        .body as Array<{ token: string; roomId: string | null }>;
      expect(again.find((c) => c.roomId === s.room1.id)!.token).toBe(s.salle1);
      expect(again).toHaveLength(3); // entrée + deux salles

      const rotated = (
        await post('/pointage-codes/rotate', { roomId: s.room1.id }).expect(201)
      ).body as Array<{ token: string; roomId: string | null }>;
      expect(rotated.find((c) => c.roomId === s.room1.id)!.token).not.toBe(
        s.salle1,
      );
      await scan(s.p1.token, s.salle1, at('08:00')).expect(404); // l'ancien QR affiché ne marche plus

      const rooms = JSON.stringify(
        (await get('/rooms', s.p1.token).expect(200)).body,
      );
      for (const t of [s.salle1, s.salle2, s.gate])
        expect(rooms).not.toContain(t);
    ...
    it('supprimer une salle supprime son QR', async () => {
      ...
      expect(
        await prisma.pointageCode.count({ where: { roomId: room.id } }),
      ).toBe(1);
    ...
    it('relie un compte à une fiche : un compte, une fiche, jamais deux', async () => {
      const s = await school();
      const users = (await get('/teachers/linkable-users').expect(200))
        .body as Array<{ id: string; teacherId: string | null }>;
      expect(users.find((u) => u.id === s.p1.user.id)!.teacherId).toBe(s.t1.id);
      const conflict = await patch(`/teachers/${s.t3.id}`, {
        userId: s.p1.user.id,
      }).expect(409);
      ...
      await patch(`/teachers/${s.t3.id}`, { modePointage: 'PAR_HEURE' }).expect(
        400,
      );
    });
    ...
    it('les règles de pointage se règlent par paramètre', async () => {
      await patch('/pedagogy/settings', {
        pointageFenetreMinutes: 30,
        pointageToleranceMinutes: 5,
        pointageEcartMinMinutes: 2,
      }).expect(200);
      ...
      await patch('/pedagogy/settings', { pointageEcartMinMinutes: 0 }).expect(
        400,
      );
    });
  });
  ...
  describe('pointage par séance (collège, lycée)', () => {
    it('début puis fin d’une séance, avec le retard calculé', async () => {
      const s = await school();
      const start = (await scan(s.p1.token, s.salle1, at('08:03')).expect(201))
        .body;
      ...
      expect(start.seance).toMatchObject({
        className: 'CM2 A',
        subjectName: 'Mathématiques',
        heureDebut: '08:00',
      });
      const end = (await scan(s.p1.token, s.salle1, at('08:49')).expect(201))
        .body;
    ...
    it('signale un retard au-delà de la tolérance paramétrée', async () => {
      const s = await school();
      const late = (await scan(s.p1.token, s.salle1, at('08:14')).expect(201))
        .body;
    ...
    it('à la jonction de deux cours, un scan clôt le premier et le suivant ouvre le second', async () => {
      ...
      const fin = (await scan(s.p1.token, s.salle2, at('08:50')).expect(201))
        .body;
      expect(fin).toMatchObject({
        type: 'FIN',
        seance: { className: 'CM2 A', subjectName: 'Mathématiques' },
      });
      const debut = (await scan(s.p1.token, s.salle2, at('08:51')).expect(201))
        .body;
      expect(debut).toMatchObject({
        type: 'DEBUT',
        retardMinutes: 1,
        seance: { subjectName: 'Français' },
      });
    ...
    it('refuse un double scan trop rapproché, mais un même scan rejoué ne change rien', async () => {
      ...
      const replay = (await scan(s.p1.token, s.salle1, at('08:03')).expect(201))
        .body;
    ...
    it('note un écart quand le QR scanné n’est pas celui de la salle prévue', async () => {
      const s = await school();
      const res = (await scan(s.p1.token, s.salle2, at('08:00')).expect(201))
        .body;
    ...
    it('refuse le mauvais type de QR, un QR inconnu, un compte non relié, un jour sans séance', async () => {
      ...
      const wrongRoom = await scan(s.p2.token, s.salle1, at('08:00')).expect(
        422,
      );
      ...
      const noSession = await scan(
        s.p1.token,
        s.salle1,
        at('08:00', addDays(yesterday, -1)),
      ).expect(422);
    ...
    it('refuse une heure de scan dans le futur ou de plus de 45 jours', async () => {
      ...
      await scan(
        s.p1.token,
        s.salle1,
        new Date(Date.now() - 46 * 86400000).toISOString(),
      ).expect(400);
    });
    ...
    it('crédite le remplaçant, jamais l’absent', async () => {
      ...
      const res = (await scan(s.p3.token, s.salle1, at('08:00')).expect(201))
        .body;
      expect(res).toMatchObject({
        type: 'DEBUT',
        seance: { subjectName: 'Mathématiques' },
      });
  ...
  describe('pointage à l’arrivée et au départ (maternelle, primaire)', () => {
    it('arrivée, puis départ (le dernier scan du jour), avec le retard sur la première séance', async () => {
      const s = await school();
      const arrival = (await scan(s.p2.token, s.gate, at('10:04')).expect(201))
        .body;
      ...
      const depart = (await scan(s.p2.token, s.gate, at('16:00')).expect(201))
        .body;
      ...
      expect(ck.departAt?.toISOString()).toBe(
  ...
  describe('validation par un tiers (RV06)', () => {
    it('le surveillant valide ou rejette ; une décision ne se rejoue pas ; l’enseignant ne décide pas', async () => {
      const s = await school();
      const surv = await userToken('SURVEILLANT', 'surv@test.local');
      const first = (await scan(s.p1.token, s.salle1, at('08:00')).expect(201))
        .body;
      await scan(s.p1.token, s.salle1, at('08:50')).expect(201);

      await post(
        `/teacher-checkins/sessions/${first.pointageId}/decision`,
        { statut: 'VALIDE' },
        s.p1.token,
      ).expect(403);
      await post(
      ...
      expect(stored).toMatchObject({
        statut: 'VALIDE',
        decidedById: surv.user.id,
      });

      const day = (await scan(s.p2.token, s.gate, at('10:00')).expect(201))
        .body;
      await post(
        `/teacher-checkins/days/${day.pointageId}/decision`,
        { statut: 'REJETE', motif: 'Scan fait hors de l’école' },
        surv.token,
      ).expect(201);
      expect(
        (await prisma.teacherDayCheckin.findFirstOrThrow()).motif,
      ).toContain('hors de l’école');
      await post(
        '/teacher-checkins/sessions/inconnu/decision',
        { statut: 'VALIDE' },
        surv.token,
      ).expect(404);
    });
    ...
    it('un validateur qui est aussi enseignant ne valide ni ne corrige jamais son propre pointage', async () => {
      ...
      const role = await prisma.role.create({
        data: { code: 'DOUBLE', nom: 'Double', description: 'test' },
      });
      ...
      const mine = (await scan(dbl.token, s.salle1, at('08:00')).expect(201))
        .body;
      const denied = await post(
        `/teacher-checkins/sessions/${mine.pointageId}/decision`,
        { statut: 'VALIDE' },
        dbl.token,
      ).expect(403);
      expect(denied.body.message).toContain('propre pointage');
      await post(
        '/teacher-checkins/sessions/manual',
        {
          entryId: s.e1.id,
          date: yesterday,
          debut: '08:00',
      ...
      expect(
        (await prisma.teacherSessionCheckin.findFirstOrThrow()).statut,
      ).toBe('EN_ATTENTE');

      // Un autre responsable, lui, peut.
      await post(`/teacher-checkins/sessions/${mine.pointageId}/decision`, {
        statut: 'VALIDE',
      }).expect(201);
    });
    ...
    it('la vie scolaire saisit ou corrige un pointage avec un motif, validé d’office et tracé', async () => {
      ...
      await post(
        '/teacher-checkins/sessions/manual',
        { ...body, fin: '08:00', motif: 'x' },
        surv.token,
      ).expect(400);
      const created = (
        await post(
          '/teacher-checkins/sessions/manual',
          { ...body, motif: 'Téléphone en panne' },
          surv.token,
        ).expect(201)
      ).body;
      ...
      await post(
        '/teacher-checkins/sessions/manual',
        { ...body, debut: '08:00', motif: 'Erreur de scan' },
        surv.token,
      ).expect(201);
      expect(await prisma.teacherSessionCheckin.count()).toBe(1);
      expect(
        (
          await prisma.teacherSessionCheckin.findFirstOrThrow()
        ).debutAt?.toISOString(),
      ).toBe(new Date(at('08:00')).toISOString());

      await post(
        '/teacher-checkins/sessions/manual',
        { ...body, date: addDays(today, 1), motif: 'x' },
        surv.token,
      ).expect(422);
      await post(
        '/teacher-checkins/sessions/manual',
        ...
        surv.token,
      ).expect(422); // t2 pointe par journée
      await post(
        '/teacher-checkins/days/manual',
        { teacherId: s.t1.id, date: yesterday, arrivee: '08:00', motif: 'x' },
        surv.token,
      ).expect(422); // t1 pointe par séance
      const day = (
        await post(
          '/teacher-checkins/days/manual',
          {
    ...
    it('refuse de saisir le pointage d’une séance annulée', async () => {
      const s = await school();
      await post(`/timetable-entries/${s.e2.id}/exceptions`, {
        date: yesterday,
        type: 'ANNULEE',
        motif: 'Grève',
      }).expect(201);
      await post('/teacher-checkins/sessions/manual', {
        entryId: s.e2.id,
        date: yesterday,
        debut: '08:50',
        fin: '09:40',
  ...
  describe('vues et récapitulatif mensuel (RV07)', () => {
    it('la journée montre les séances non pointées, et l’enseignant voit son propre état', async () => {
      ...
      const day = (
        await get(`/teacher-checkins/day?date=${yesterday}`).expect(200)
      ).body;
      const e1 = day.seances.find(
        (x: { entryId: string }) => x.entryId === s.e1.id,
      );
      const e2 = day.seances.find(
        (x: { entryId: string }) => x.entryId === s.e2.id,
      );
      expect(e1.pointage).toMatchObject({
        debut: '08:00',
        statut: 'EN_ATTENTE',
      });
      expect(e2.pointage).toBeNull();
      expect(
        day.seances.find((x: { entryId: string }) => x.entryId === s.e3.id),
      ).toBeUndefined(); // t2 pointe par journée
      expect(day.journees).toHaveLength(1);
      expect(day.journees[0]).toMatchObject({
        teacherName: 'Claire Mabiala',
        prevue: true,
      });
      await get('/teacher-checkins/day?date=hier').expect(400);

      const me = (await get('/teacher-checkins/me', s.p2.token).expect(200))
        .body;
    ...
    it('calcule les heures effectuées d’après les seuls pointages validés', async () => {
      ...
      const a = (await scan(s.p1.token, s.salle1, at('08:14')).expect(201))
        .body;
      await scan(s.p1.token, s.salle1, at('08:50')).expect(201);
      await scan(s.p1.token, s.salle2, at('08:52')).expect(201);
      await post(
        `/teacher-checkins/sessions/${a.pointageId}/decision`,
        { statut: 'VALIDE' },
        surv.token,
      ).expect(201);
      ...
      await post(
        `/teacher-checkins/days/${d.pointageId}/decision`,
        { statut: 'VALIDE' },
        surv.token,
      ).expect(201);
      ...
      const sum = (
        await get(
          `/teacher-checkins/summary?month=${month}`,
          surv.token,
        ).expect(200)
      ).body;
      const row = (name: string) =>
        sum.enseignants.find(
          (r: { enseignant: string }) => r.enseignant === name,
        );
      ...
      expect(t1).toMatchObject({
        ...
        seancesTenues: 1,
        seancesEnAttente: 1,
        seancesNonPointees: 0,
        minutesEffectuees: 50,
        retards: 1,
        minutesRetard: 14,
      });
      ...
      await get('/teacher-checkins/summary?month=2026-13', surv.token).expect(
        400,
      );
      const one = (
        await get(
          `/teacher-checkins/summary?month=${month}&teacherId=${s.t1.id}`,
          surv.token,
        ).expect(200)
      ).body;
    ...
    it('note l’absent comme ayant confié une séance à un remplaçant, et crédite le remplaçant', async () => {
      ...
      const r = (await scan(s.p3.token, s.salle1, at('08:00')).expect(201))
        .body;
      await scan(s.p3.token, s.salle1, at('08:50')).expect(201);
      await post(
        `/teacher-checkins/sessions/${r.pointageId}/decision`,
        { statut: 'VALIDE' },
        surv.token,
      ).expect(201);
      ...
      const sum = (
        await get(
          `/teacher-checkins/summary?month=${yesterday.slice(0, 7)}`,
          surv.token,
        ).expect(200)
      ).body;
      const row = (name: string) =>
        sum.enseignants.find(
          (x: { enseignant: string }) => x.enseignant === name,
        );
      expect(row('Jean Okemba')).toMatchObject({
        seancesTenues: 1,
        minutesEffectuees: 50,
      });
      expect(row('Paul Ngoma')).toMatchObject({
        seancesPrevues: 1,
        confieesARemplacant: 1,
      });
    });
    ...
    it('trace les codes, la décision et le lien de compte dans le journal d’audit', async () => {
      ...
      const first = (await scan(s.p1.token, s.salle1, at('08:00')).expect(201))
        .body;
      await post(`/teacher-checkins/sessions/${first.pointageId}/decision`, {
        statut: 'VALIDE',
      }).expect(201);
      const actions = (
        await prisma.auditLog.findMany({ select: { action: true } })
