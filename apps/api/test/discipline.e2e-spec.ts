...
describe('Discipline (e2e, Lot 20)', () => {
  ...
  const SECRET_TEXT =
    'Il a frappé un camarade dans la cour pendant la récréation';
  ...
  async function world(): Promise<World> {
    ...
    const guardian = async (nom: string, tel: string, studentId: string) => {
      const g = await prisma.guardian.create({
        data: { schoolId: sch.id, nom, prenom: 'Parent', telephone: tel },
      });
      await prisma.studentGuardian.create({
        data: { studentId, guardianId: g.id, lien: 'Mère' },
      });
      return g;
    };
    ...
    const incidentType = (
      await post('/discipline/types', admin, {
        nature: 'INCIDENT',
        nom: 'Bagarre',
      }).expect(201)
    ).body.id;
    ...
    const sanctionType = (
      await post('/discipline/sanction-types', admin, {
        nom: 'Retenue',
      }).expect(201)
    ).body.id;
    return {
  ...
  const pget = (path: string, t: string) =>
    request(app.getHttpServer()).get(path).set('Authorization', `Bearer ${t}`);
  const ppost = (path: string, t: string) =>
    request(app.getHttpServer())
      .post(path)
      .set('Authorization', `Bearer ${t}`)
      .send({});
  ...
  async function withSanction(w: World, publish = false) {
    const record = (
      await post('/discipline/records', w.prof, incident(w, w.alice.id)).expect(
        201,
      )
    ).body;
    const sanction = (
    ...
    if (publish)
      await post(`/discipline/sanctions/${sanction.id}/publier`, w.dir).expect(
        201,
      );
  ...
  describe('catalogues', () => {
    it('sont vides au départ, gérés par la pédagogie, sans doublon, et un type désactivé n’est plus proposé', async () => {
      const w = await world();
      const types = (await get('/discipline/types', w.prof).expect(200))
        .body as Array<{ nom: string }>;
      expect(types.map((t) => t.nom).sort()).toEqual([
        'Bagarre',
        'Félicitations',
      ]);
      await post('/discipline/types', admin, {
      ...
      await post('/discipline/types', w.prof, {
        nature: 'INCIDENT',
        nom: 'Vol',
      }).expect(403);
      await post('/discipline/sanction-types', w.surv, { nom: 'Blâme' }).expect(
        403,
      );
      await patch(`/discipline/types/${w.incidentType}`, admin, {
        actif: false,
      }).expect(200);
      const res = await post(
        '/discipline/records',
        w.prof,
        incident(w, w.alice.id),
      ).expect(400);
  ...
  describe('droits et portée', () => {
    it('ni l’Administrateur, ni l’Auditeur, ni le comptable, ni sans jeton', async () => {
      ...
      for (const t of [admin, aud.token, cpt.token]) {
        ...
        await post('/discipline/records', t, incident(w, w.alice.id)).expect(
          403,
        );
    ...
    it('un enseignant ne signale que pour ses classes ; la vie scolaire et la Direction pour toute l’école', async () => {
      ...
      await post('/discipline/records', w.dir, incident(w, w.alice.id)).expect(
        201,
      );
    });
    ...
    it('les sélecteurs de l’enseignant se limitent à ses classes', async () => {
      const w = await world();
      const mine = (await get('/discipline/my-classes', w.prof).expect(200))
        .body as Array<{ id: string }>;
      expect(mine.map((c) => c.id)).toEqual([w.klassA.id]);
      const all = (await get('/discipline/my-classes', w.surv).expect(200))
        .body as Array<{ id: string }>;
      expect(all).toHaveLength(2);
      const students = (
        await get(`/discipline/classes/${w.klassA.id}/students`, w.prof).expect(
          200,
        )
      ).body as Array<{ id: string }>;
      expect(students.map((s) => s.id).sort()).toEqual(
        [w.alice.id, w.brice.id].sort(),
      );
      await get(`/discipline/classes/${w.klassB.id}/students`, w.prof).expect(
        403,
      );
    });
    ...
    it('les droits sont attribués comme prévu, et la décision est un droit réservé', async () => {
      ...
      const by = (code: string) =>
        rows
          .filter((r) => r.permission.code === code)
          .map((r) => r.role.code)
          .sort();
      expect(by('DISCIPLINE_REPORT')).toEqual([
        'DIRECTION',
        'ENSEIGNANT',
        'SURVEILLANT',
      ]);
      ...
      const sql = readFileSync(
        join(
          ...
          'prisma',
          'migrations',
          '20260925080000_lot20_discipline',
          'migration.sql',
        ),
        'utf8',
      );
      ...
      const roles = (await get('/roles', admin).expect(200)).body as Array<{
        id: string;
        code: string;
        permissions: string[];
      }>;
      ...
      await put(`/roles/${direction.id}/permissions`, admin, {
        permissionCodes: direction.permissions.filter(
          (c) => c !== 'DISCIPLINE_DECIDE',
        ),
      }).expect(403);
      const adminRole = roles.find((r) => r.code === 'ADMINISTRATEUR')!;
      expect(
        adminRole.permissions.some((c) => c.startsWith('DISCIPLINE_')),
      ).toBe(false);
    });
  });
  ...
  describe('signalements', () => {
    it('valide la nature, la gravité, la description, la date et l’inscription', async () => {
      ...
      const bad = (over: object) =>
        post('/discipline/records', w.dir, incident(w, w.alice.id, over));
      ...
      const noEnrollment = await post(
        '/discipline/records',
        w.dir,
        incident(w, w.diane.id),
      ).expect(422);
      expect(noEnrollment.body.message).toMatch(/inscription active/);
      await post('/discipline/records', w.dir, incident(w, 'inconnu')).expect(
        404,
      );
      // Un numéro de téléphone n'a rien à faire dans un dossier.
      const phone = await bad({
        description: 'Appeler le père au 06 12 34 56 78 pour lui parler',
      }).expect(422);
    ...
    it('un enseignant ne relit que ses signalements, sans le message à la famille ni une sanction non publiée', async () => {
      ...
      const own = (await get('/discipline/records', w.prof).expect(200))
        .body as Array<{ id: string; sanctions: unknown[] }>;
      expect(own.map((r) => r.id)).toEqual([record.id]);
      expect(own[0].sanctions).toHaveLength(0); // sanction décidée, pas encore publiée
      const all = (await get('/discipline/records', w.surv).expect(200))
        .body as unknown[];
      expect(all).toHaveLength(2);
      const filtered = (
        await get(`/discipline/records?studentId=${w.carine.id}`, w.dir).expect(
          200,
        )
      ).body as unknown[];
      ...
      const after = (await get('/discipline/records', w.prof).expect(200))
        .body as Array<{ sanctions: Array<Record<string, unknown>> }>;
      ...
      const full = (await get('/discipline/records', w.dir).expect(200))
        .body as Array<{ sanctions: Array<Record<string, unknown>> }>;
    ...
    it('l’auteur corrige le jour même ; ensuite seule la Direction, avec un motif et une révision conservée', async () => {
      ...
      await patch(`/discipline/records/${record.id}`, w.prof, {
        gravite: 'GRAVE',
      }).expect(200);
      expect(await prisma.disciplineRecordRevision.count()).toBe(0); // correction du jour : pas de révision
      await patch(`/discipline/records/${record.id}`, w.profB, {
        gravite: 'LEGER',
      }).expect(403); // pas l'auteur
      ...
      const locked = await patch(`/discipline/records/${record.id}`, w.prof, {
        gravite: 'LEGER',
      }).expect(403);
      ...
      expect(revisions[0].avant).toMatchObject({
        gravite: 'GRAVE',
        description: SECRET_TEXT,
      });
      // Le surveillant ne corrige pas un signalement verrouillé.
      await patch(`/discipline/records/${record.id}`, w.surv, {
        gravite: 'MOYEN',
    ...
    it('classer sans suite, annuler : réservés à la Direction, avec motif', async () => {
      ...
      await post(`/discipline/records/${r1.id}/classer`, w.surv, {
        motif: 'Sans gravité',
      }).expect(403);
      await post(`/discipline/records/${r1.id}/classer`, w.dir, {
        motif: 'x',
      }).expect(400);
      await post(`/discipline/records/${r1.id}/classer`, w.dir, {
        motif: 'Sans gravité, réglé en classe',
      }).expect(201);
      // Un incident classé sans suite ne reçoit plus de sanction.
      await post(`/discipline/records/${r1.id}/sanctions`, w.dir, {
        typeId: w.sanctionType,
        dateDebut: today,
      }).expect(409);
      await post(`/discipline/records/${r1.id}/annuler`, w.prof, {
        motif: 'Erreur de saisie',
      }).expect(403);
      await post(`/discipline/records/${r1.id}/annuler`, w.dir, {
        motif: 'Erreur de saisie',
      }).expect(201);
      await post(`/discipline/records/${r1.id}/annuler`, w.dir, {
        motif: 'Encore',
      }).expect(409);
      await patch(`/discipline/records/${r1.id}`, w.dir, {
        gravite: 'LEGER',
        motif: 'Correction',
      }).expect(409);
      await post(`/discipline/records/${r1.id}/sanctions`, w.dir, {
  ...
  describe('sanctions', () => {
    it('seule la Direction décide, publie et annule', async () => {
      ...
      for (const t of [w.prof, w.surv, admin]) {
        await post(
          `/discipline/records/${record.id}/sanctions`,
          t,
          body,
        ).expect(403);
      }
      ...
      expect(
        (
          await prisma.disciplineRecord.findUniqueOrThrow({
            where: { id: record.id },
          })
        ).statut,
      ).toBe('TRAITE');
      for (const t of [w.prof, w.surv, admin]) {
        await post(`/discipline/sanctions/${sanction.id}/publier`, t).expect(
          403,
        );
        await post(`/discipline/sanctions/${sanction.id}/annuler`, t, {
          motif: 'Erreur',
        }).expect(403);
      ...
      await post(`/discipline/records/${record.id}/annuler`, w.dir, {
        motif: 'Erreur de saisie',
      }).expect(409);
      await post(`/discipline/sanctions/${sanction.id}/annuler`, w.dir, {
        motif: 'Décision retirée',
      }).expect(201);
      await post(`/discipline/sanctions/${sanction.id}/annuler`, w.dir, {
        motif: 'Encore',
      }).expect(409);
      ...
      await post('/discipline/records', w.dir, {
        studentId: w.alice.id,
        nature: 'VALORISATION',
        typeId: w.praiseType,
        dateFaits: today,
      }).expect(201);
      const praise = await prisma.disciplineRecord.findFirstOrThrow({
        where: { nature: 'VALORISATION' },
      });
      await post(
        `/discipline/records/${praise.id}/sanctions`,
        w.dir,
        body,
      ).expect(400);
    });
    ...
    it('la Direction liste les sanctions à publier', async () => {
      ...
      const pending = (
        await get('/discipline/sanctions?statut=DECIDEE', w.dir).expect(200)
      ).body as Array<{ eleve: { id: string }; messageFamille: string }>;
      expect(pending).toHaveLength(1);
      expect(pending[0].eleve.id).toBe(w.alice.id);
      expect(
        (await get('/discipline/sanctions?statut=PUBLIEE', w.dir).expect(200))
          .body,
      ).toHaveLength(0);
  ...
  describe('portail des parents', () => {
    it('une sanction n’est visible qu’une fois publiée, puis « retirée » si elle est annulée', async () => {
      ...
      const before = (
        await pget(`/portal/children/${w.alice.id}/discipline`, parent).expect(
          200,
        )
      ).body;
    ...
    it('la réponse est une liste blanche : jamais la description, l’auteur, la gravité, l’incident ni un autre élève', async () => {
      ...
      await post(
        '/discipline/records',
        w.prof,
        incident(w, w.brice.id, {
          description: 'Incident concernant un autre élève',
        }),
      ).expect(201);
      await post('/discipline/records', w.prof, {
        studentId: w.alice.id,
      ...
      await post('/discipline/convocations', w.surv, {
        studentId: w.alice.id,
        recordId: record.id,
        dateRdv: new Date(Date.now() + 2 * 86400000).toISOString(),
        lieu: 'Bureau',
        objet: 'Entretien',
      }).expect(201);
      const body = (
      ...
      expect(Object.keys(body.sanctions[0]).sort()).toEqual([
        'dateDebut',
        'dateFin',
        'id',
        'message',
      ...
      expect(Object.keys(body.convocations[0]).sort()).toEqual([
        'accuseLe',
        'annulee',
        'dateRdv',
        'id',
      ...
      expect(Object.keys(body.valorisations[0]).sort()).toEqual([
        'date',
        'id',
        'type',
      ]);
    ...
    it('un parent n’accède jamais à l’enfant d’une autre famille ; le personnel n’ouvre pas le portail', async () => {
      const w = await world();
      const parent = await parentToken(w.moukala.id, '242060000001');
      await pget(`/portal/children/${w.carine.id}/discipline`, parent).expect(
        404,
      );
      await request(app.getHttpServer())
        .get(`/portal/children/${w.alice.id}/discipline`)
        .expect(401);
      await pget(`/portal/children/${w.alice.id}/discipline`, w.dir).expect(
        401,
      );
      // Un accès retiré à ce responsable pour cet élève ferme aussi cet onglet.
      await prisma.studentGuardian.updateMany({
        where: { guardianId: w.moukala.id },
    ...
    it('les valorisations sont visibles sans alerte, une valorisation annulée disparaît', async () => {
      ...
      expect(
        (
          await pget(
            `/portal/children/${w.alice.id}/discipline`,
            parent,
          ).expect(200)
        ).body.valorisations,
      ).toHaveLength(1);
      ...
      await post(`/discipline/records/${praise.id}/annuler`, w.dir, {
        motif: 'Saisie par erreur',
      }).expect(201);
      expect(
        (
          await pget(
  ...
  describe('alerte générique', () => {
    it('la publication d’une sanction prévient sans jamais dire de quoi il s’agit', async () => {
      ...
      await request(app.getHttpServer())
        ...
        .send({
          endpoint: 'https://push.example.test/d1',
          keys: { p256dh: 'clePublique', auth: 'secretAuth' },
        })
        .expect(201);
      const { sanction } = await withSanction(w);
      await notifications.idle();
      ...
      expect(body).toBe(
        "Un nouvel élément de vie scolaire est disponible pour Alice. Ouvrez l'application pour le consulter.",
      );
      for (const secret of [
        'Bagarre',
        'Retenue',
      ...
      for (const secret of [
        ...
        expect(rows[0].corps).not.toContain(secret);
    });
    ...
    it('seuls les responsables de l’élève sont prévenus', async () => {
      ...
      await notifications.idle();
      const a = (await pget('/portal/notifications', parentA).expect(200)).body
        .notifications as unknown[];
      const b = (await pget('/portal/notifications', parentB).expect(200)).body
        .notifications as unknown[];
    ...
    it('un service push en panne ne fait pas échouer la publication', async () => {
      ...
      expect(
        (
          await prisma.sanction.findUniqueOrThrow({
  ...
  describe('convocations', () => {
    ...
    it('se créent par la vie scolaire, avec validation, et préviennent la famille sans détail', async () => {
      ...
      expect(view.convocations[0]).toMatchObject({
        lieu: 'Bureau de la vie scolaire',
        objet: 'Entretien avec la famille',
        annulee: false,
        accuseLe: null,
    ...
    it('le parent accuse réception (idempotent) ; annulée, elle ne s’accuse plus ; l’issue se pose une fois', async () => {
      ...
      await ppost(
        `/portal/children/${w.alice.id}/discipline/convocations/${c.id}/accuser`,
        otherParent,
      ).expect(404);
      const first = (
        await ppost(
      ...
      const stored = await prisma.disciplineConvocation.findUniqueOrThrow({
        where: { id: c.id },
      });
      expect(stored.accuseParGuardianId).toBe(w.moukala.id);
      await ppost(
        `/portal/children/${w.alice.id}/discipline/convocations/inconnue/accuser`,
        parent,
      ).expect(404);
      await ppost(
        `/portal/children/${w.carine.id}/discipline/convocations/${c.id}/accuser`,
        parent,
      ).expect(404);

      await post(`/discipline/convocations/${c.id}/issue`, w.prof, {
        issue: 'PRESENT',
      }).expect(403);
      await post(`/discipline/convocations/${c.id}/issue`, w.surv, {
        issue: 'PEUT-ETRE',
      }).expect(400);
      await post(`/discipline/convocations/${c.id}/issue`, w.surv, {
        issue: 'PRESENT',
      ...
      await post(`/discipline/convocations/${c2.id}/annuler`, w.surv, {
        motif: 'x',
      }).expect(400);
      await post(`/discipline/convocations/${c2.id}/annuler`, w.surv, {
        motif: 'Report du rendez-vous',
      }).expect(201);
      await post(`/discipline/convocations/${c2.id}/annuler`, w.surv, {
        motif: 'Encore',
      }).expect(409);
      await ppost(
        `/portal/children/${w.alice.id}/discipline/convocations/${c2.id}/accuser`,
        parent,
      ).expect(409);
      await post(`/discipline/convocations/${c2.id}/issue`, w.surv, {
        issue: 'PRESENT',
      }).expect(409);
      const view = (
        await pget(`/portal/children/${w.alice.id}/discipline`, parent).expect(
      ...
      expect(
        view.convocations.find((x: { id: string }) => x.id === c2.id).annulee,
      ).toBe(true);
      const listed = (
        await get(
          `/discipline/convocations?studentId=${w.alice.id}`,
          w.surv,
        ).expect(200)
      ).body as unknown[];
  ...
  describe('dossier et journal (RV11)', () => {
    it('le dossier d’un élève est réservé à la vie scolaire et à la Direction, et chaque ouverture est journalisée', async () => {
      ...
      await get(`/discipline/students/${w.alice.id}/history`, w.prof).expect(
        403,
      );
      await get(`/discipline/students/${w.alice.id}/history`, admin).expect(
        403,
      );
      await get(`/discipline/students/inconnu/history`, w.surv).expect(404);
      const history = (
        await get(`/discipline/students/${w.alice.id}/history`, w.surv).expect(
          200,
        )
      ).body;
      expect(history.signalements).toHaveLength(1);
      expect(history.signalements[0].description).toBe(SECRET_TEXT);
      expect(history.signalements[0].sanctions[0].messageFamille).toBe(
        FAMILY_TEXT,
      );
      await get(`/discipline/students/${w.alice.id}/history`, w.dir).expect(
        200,
      );
      const reads = await prisma.auditLog.findMany({
        where: { action: 'DISCIPLINE_STUDENT_READ' },
      });
    ...
    it('le journal ne contient jamais la description, le message à la famille, le lieu ni l’objet d’une convocation', async () => {
      ...
      const rows = await prisma.auditLog.findMany({
        where: { action: { startsWith: 'DISCIPLINE_' } },
      });
      ...
      for (const a of [
        ...
        'DISCIPLINE_SANCTION_PUBLISH',
        'DISCIPLINE_SANCTION_CANCEL',
      ...
      for (const secret of [
        SECRET_TEXT,
        FAMILY_TEXT,
        'Nouvelle description confidentielle',
        'Bureau confidentiel',
        'Objet confidentiel',
      ]) {
