...
describe('Assiduité des élèves (e2e, Lot 9)', () => {
  ...
  async function takeOnlyToken(email = 'prof@test.local') {
    ...
    if (!role) {
      role = await prisma.role.create({
        data: { code: 'APPEL_SEUL', nom: 'Appel seul', description: 'test' },
      });
      const perms = await prisma.permission.findMany({
        where: { code: { in: ['ATTENDANCE_TAKE', 'ATTENDANCE_READ'] } },
      });
  ...
  async function school() {
    ...
    const t2 = (await post('/teachers', { nom: 'Okemba', prenom: 'Jean' }))
      .body;
  ...
  describe('feuille d’appel', () => {
    it('affiche toute la classe présente par défaut, par ordre alphabétique, sans les inscriptions annulées', async () => {
      ...
      await prisma.enrollment.updateMany({
        where: { studentId: s.s2.id },
        data: { statut: 'ANNULEE' },
      });
      const sheet = (
        await get(
          `/attendance/sheet?entryId=${s.entryToday.id}&date=${today}`,
        ).expect(200)
      ).body;
      expect(sheet.eleves.map((e: { nom: string }) => e.nom)).toEqual([
        'Bakala',
        'Zola',
      ]);
      expect(
        sheet.eleves.every((e: { statut: string }) => e.statut === 'PRESENT'),
      ).toBe(true);
  ...
  describe('saisie d’un appel (RV05, D57)', () => {
    it('calcule le statut d’après le seuil paramétré : présent, retard, absent', async () => {
      ...
      const by = (id: string) =>
        res.body.eleves.find((e: { studentId: string }) => e.studentId === id);
      expect(by(s.s1.id)).toMatchObject({
        statut: 'RETARD',
        minutesRetard: 10,
      });
      expect(by(s.s2.id)).toMatchObject({
        statut: 'ABSENT',
        minutesRetard: 20,
      });
      expect(by(s.s3.id)).toMatchObject({
        statut: 'ABSENT',
        minutesRetard: null,
      });
    });
    ...
    it('le seuil se change par paramètre, sans toucher au code', async () => {
      ...
      expect(
        res.body.eleves.find(
          (e: { studentId: string }) => e.studentId === s.s1.id,
        ).statut,
      ).toBe('ABSENT');
      await patch('/pedagogy/settings', { retardMaxMinutes: 0 }).expect(400);
      await patch('/pedagogy/settings', { delaiJustificatifJours: 5 }).expect(
        200,
      );
      expect(
        (await get('/pedagogy/settings').expect(200)).body
          .delaiJustificatifJours,
      ).toBe(5);
    });
    ...
    it('renvoyer le même appel ne crée jamais de doublon (synchronisation hors ligne)', async () => {
      const s = await school();
      const body = {
        entryId: s.entryToday.id,
        date: today,
        absences: [{ studentId: s.s1.id, absent: true }],
      };
      const send = () =>
        request(app.getHttpServer())
    ...
    it('l’auteur modifie son appel du jour sans motif ; les élèves non listés redeviennent présents', async () => {
      ...
      const res = await call(s.entryToday.id, today, [
        { studentId: s.s2.id, minutesRetard: 3 },
      ]).expect(201);
      const by = (id: string) =>
        res.body.eleves.find((e: { studentId: string }) => e.studentId === id);
    ...
    it('refuse les saisies incohérentes', async () => {
      const s = await school();
      await call(s.entryToday.id, today, [
        { studentId: 'inconnu', absent: true },
      ]).expect(422);
      await call(s.entryToday.id, today, [
        { studentId: s.s1.id, absent: true },
        { studentId: s.s1.id, minutesRetard: 5 },
      ]).expect(400);
      await call(s.entryToday.id, today, [{ studentId: s.s1.id }]).expect(400);
      await call(s.entryToday.id, today, [
        { studentId: s.s1.id, minutesRetard: 60 },
      ]).expect(422); // séance de 50 minutes
    ...
    it('refuse l’appel d’une séance annulée et note le remplaçant d’une séance remplacée', async () => {
      ...
      await call(s.entryYesterday.id, yesterday, [], {
        motif: 'Appel oublié',
      }).expect(201);
      const saved = await prisma.attendanceCall.findFirstOrThrow({
        where: { entryId: s.entryYesterday.id },
      });
  ...
  describe('verrouillage et corrections (RV04, D59)', () => {
    it('un autre utilisateur ne modifie pas l’appel du jour ; la vie scolaire le corrige avec un motif tracé', async () => {
      ...
      await call(s.entryToday.id, today, []).expect(422);
      await call(s.entryToday.id, today, [], {
        motif: 'Élève arrivé avec un mot du médecin',
      }).expect(201);
    ...
    it('après la fin de la journée, l’auteur ne corrige plus : seule la vie scolaire, avec motif', async () => {
      ...
      await call(
        s.entryYesterday.id,
        yesterday,
        [{ studentId: s.s2.id, minutesRetard: 8 }],
      ...
      expect(
        corrections.map((c) => `${c.ancienStatut}>${c.nouveauStatut}`).sort(),
      ).toEqual(['ABSENT>PRESENT', 'PRESENT>RETARD']);

      const history = (
        await get(`/attendance/students/${s.s1.id}/history`).expect(200)
      ).body;
      expect(history.lignes).toHaveLength(0); // plus absent
      const history2 = (
        await get(`/attendance/students/${s.s2.id}/history`).expect(200)
      ).body;
      expect(history2.lignes[0].corrections[0]).toMatchObject({
        de: 'PRESENT',
        vers: 'RETARD',
        motif: 'Erreur de saisie',
      });
  ...
  describe('justificatifs (D58)', () => {
    ...
    it('enregistre, puis accepte ou refuse un justificatif ; une seule décision', async () => {
      ...
      const created = (
        await post(`/attendance/records/${absent}/justification`, {
          reasonId: reason.id,
        }).expect(201)
      ).body;
      ...
      const decided = (
        await patch(`/attendance/justifications/${created.id}`, {
          statut: 'ACCEPTEE',
        }).expect(200)
      ).body;
    ...
    it('refuse de justifier un élève présent, et un motif désactivé', async () => {
      ...
      await post(`/attendance/records/${present}/justification`, {
        commentaire: 'x',
      }).expect(422);
      // Le présent par défaut n'a pas de ligne tant que rien n'est saisi ? Si : l'appel crée tout le monde.
      const reason = (
        await post('/absence-reasons', { libelle: 'Décès' }).expect(201)
      ).body;
      await patch(`/absence-reasons/${reason.id}`, { actif: false }).expect(
        200,
      );
      await post(`/attendance/records/${absent}/justification`, {
        reasonId: reason.id,
      }).expect(409);
      await post('/attendance/records/inconnu/justification', {
        commentaire: 'x',
      }).expect(404);
    });
    ...
    it('signale « hors délai » un justificatif déposé après le délai paramétré', async () => {
      ...
      const record = res.body.eleves.find(
        (e: { studentId: string }) => e.studentId === s.s1.id,
      ).recordId;
      const late = (
        await post(`/attendance/records/${record}/justification`, {
          commentaire: 'Certificat',
        }).expect(201)
      ).body;
    ...
    it('la décision et la saisie exigent ATTENDANCE_CORRECT : un enseignant ne justifie pas', async () => {
      ...
      const prof = await takeOnlyToken();
      await post(
        `/attendance/records/${absent}/justification`,
        { commentaire: 'x' },
        prof.token,
      ).expect(403);
      const created = (
      ...
      await patch(
        `/attendance/justifications/${created.id}`,
        { statut: 'ACCEPTEE' },
        prof.token,
      ).expect(403);
    });
    ...
    it('la liste des absences se filtre par justificatif et l’historique compte les excusées', async () => {
      ...
      await patch(`/attendance/justifications/${j.id}`, {
        statut: 'ACCEPTEE',
      }).expect(200);

      const none = (
        await get('/attendance/absences?justification=aucune').expect(200)
      ).body;
      expect(none.map((r: { recordId: string }) => r.recordId)).toEqual([
        retard,
      ]);
      const accepted = (
        await get('/attendance/absences?justification=acceptee').expect(200)
      ).body;
      expect(accepted.map((r: { recordId: string }) => r.recordId)).toEqual([
        absent,
      ]);
      await get('/attendance/absences?justification=bof').expect(400);
      const byClass = (
        await get(
          `/attendance/absences?classId=${s.klass.id}&from=${today}&to=${today}`,
        ).expect(200)
      ).body;
      ...
      expect(h1.compteurs).toMatchObject({
        absences: 1,
        retards: 0,
        excusees: 1,
        nonJustifiees: 0,
      ...
      expect(h2.compteurs).toMatchObject({
        absences: 0,
        retards: 1,
        excusees: 0,
        nonJustifiees: 1,
  ...
  describe('motifs d’absence', () => {
    it('la Direction saisit les motifs ; doublon, suppression d’un motif utilisé et permissions', async () => {
      ...
      await post('/absence-reasons', { libelle: 'Voyage' }, surv.token).expect(
        403,
      );
      ...
      const other = (
        await post('/absence-reasons', { libelle: 'Voyage' }).expect(201)
      ).body;
  ...
  describe('journal d’audit', () => {
    it('trace l’appel, la correction, le justificatif et sa décision', async () => {
      ...
      await call(s.entryToday.id, today, [
        { studentId: s.s1.id, absent: true },
        { studentId: s.s2.id, minutesRetard: 4 },
      ]).expect(201); // même auteur, même jour : simple mise à jour
      const record = res.body.eleves.find(
        (e: { studentId: string }) => e.studentId === s.s1.id,
      ).recordId;
      const j = (
      ...
      await call(s.entryYesterday.id, yesterday, [], { motif: 'Oubli' }).expect(
        201,
      ); // saisie après coup : correction
