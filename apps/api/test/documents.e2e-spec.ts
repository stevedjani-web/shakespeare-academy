...
describe('Documents officiels (e2e, Lot 19)', () => {
  ...
  const setSigner = () =>
    patch('/school', admin, {
      directeurNom: 'Marie Ndinga',
      directeurTitre: 'La Directrice',
      ville: 'Brazzaville',
    }).expect(200);
  ...
  async function school() {
    ...
    return {
      ...
      carine,
      diane,
      moukala: g('242060000001'),
      zola: g('242060000002'),
      mk,
    };
  }

  const issue = (studentId: string, body: object, t = admin) =>
    post(`/students/${studentId}/documents`, t, body);
  ...
  describe('émission', () => {
    ...
    it('fige le contenu, numérote sans le montrer dans le jeton, et garde le lieu de naissance', async () => {
      ...
      expect(doc.snapshot.eleve).toMatchObject({
        nom: 'Moukala',
        prenom: 'Alice',
        dateNaissance: '2015-04-12',
        lieuNaissance: 'Pointe-Noire',
      ...
      expect(doc.snapshot.directeur).toMatchObject({
        nom: 'Marie Ndinga',
        titre: 'La Directrice',
        ville: 'Brazzaville',
      });
      ...
      await patch('/school', admin, { directeurNom: 'Autre Personne' }).expect(
        200,
      );
    ...
    it('une demande simultanée ne crée qu’un seul document', async () => {
      ...
      const results = await Promise.all(
        [1, 2, 3, 4].map(() => issue(s.carine.id, ATT)),
      );
    ...
    it('renouveler crée un nouveau numéro et annule l’ancien document', async () => {
      ...
      const renewed = (
        await issue(s.alice.id, { ...CARTE, renouveler: true }).expect(201)
      ).body;
      expect(renewed.id).not.toBe(first.id);
      expect(renewed.numero).toBe(`CAR-${year}-000002`);
      const list = (await get(`/students/${s.alice.id}/documents`).expect(200))
        .body as Array<{ id: string; annuleLe: string | null }>;
      ...
      expect((await issue(s.alice.id, CARTE).expect(201)).body.id).toBe(
        renewed.id,
      );
    });
    ...
    it('refuse sans inscription active, pour un élève inactif ou inconnu', async () => {
      ...
      await patch(`/students/${s.alice.id}`, admin, {
        statut: 'INACTIF',
      }).expect(200);
    ...
    it('n’écrit dans le journal qu’à la création, pas à la réimpression', async () => {
      ...
      await issue(s.alice.id, CARTE).expect(201);
      expect(
        await prisma.auditLog.count({ where: { action: 'DOCUMENT_ISSUE' } }),
      ).toBe(1);
    });
  });
  ...
  describe('vérification publique', () => {
    it('ne révèle que ce qui est imprimé, jamais date de naissance, matricule ni responsable', async () => {
      ...
      const doc = (await issue(s.alice.id, ATT).expect(201)).body;
      const res = await request(app.getHttpServer())
        .get(`/documents/verify/${doc.verificationToken}`)
        .expect(200);
    ...
    it('un document annulé est signalé, un jeton inconnu ou un numéro ne donnent rien', async () => {
      ...
      await post(`/documents/${doc.id}/annuler`, dir, {
        motif: 'Carte perdue',
      }).expect(201);
      const res = await request(app.getHttpServer())
        .get(`/documents/verify/${doc.verificationToken}`)
      ...
      await request(app.getHttpServer())
        .get('/documents/verify/inconnu')
        .expect(404);
      await request(app.getHttpServer())
        .get(`/documents/verify/${doc.numero}`)
        .expect(404);
    });
  });
  ...
  describe('droits', () => {
    it('le secrétariat, la Direction et l’Administrateur émettent ; les autres non', async () => {
      ...
      await request(app.getHttpServer())
        .post(`/students/${s.alice.id}/documents`)
        .send(ATT)
        .expect(401);
    ...
    it('seule la Direction annule, avec un motif, et une seule fois', async () => {
      ...
      const dir = await tokenFor('DIRECTION', 'dir@test.local');
      const doc = (await issue(s.alice.id, CARTE).expect(201)).body;
      await post(`/documents/${doc.id}/annuler`, sec, {
        motif: 'Erreur de saisie',
      }).expect(403);
      await post(`/documents/${doc.id}/annuler`, admin, {
        motif: 'Erreur de saisie',
      }).expect(403); // droit réservé
      await post(`/documents/${doc.id}/annuler`, dir, { motif: 'x' }).expect(
        400,
      );
      const done = (
        await post(`/documents/${doc.id}/annuler`, dir, {
      ...
      await post('/documents/inconnu/annuler', dir, {
        motif: 'Erreur de saisie',
      }).expect(404);
      expect(
        await prisma.auditLog.count({ where: { action: 'DOCUMENT_CANCEL' } }),
      ).toBe(1);
    });
    ...
    it('les droits sont bien attribués par la migration et le seed', async () => {
      const rows = await prisma.rolePermission.findMany({
        where: {
          permission: { code: { in: ['DOCUMENT_ISSUE', 'DOCUMENT_CANCEL'] } },
        },
      ...
      const by = (code: string) =>
        rows
          ...
          .map((r) => r.role.code)
          .sort();
      expect(by('DOCUMENT_ISSUE')).toEqual([
        'ADMINISTRATEUR',
        'DIRECTION',
        'SECRETAIRE_CAISSIER',
      ]);
      ...
      const sql = readFileSync(
        join(
          ...
          'prisma',
          'migrations',
          '20260924080000_lot19_documents_officiels',
          'migration.sql',
        ),
        'utf8',
      );
  ...
  describe('portail parent', () => {
    it('le parent obtient l’attestation de son enfant, la même que celle du secrétariat', async () => {
      ...
      const doc = (
        await portalPost(
          `/portal/children/${s.alice.id}/attestation`,
          parent,
        ).expect(201)
      ).body;
      ...
      const mine = (
        await portalPost(
          `/portal/children/${s.carine.id}/attestation`,
          parent2,
        ).expect(201)
      ).body;
      expect(mine.emisParType).toBe('PARENT');
      expect(mine.emisParId).toBeUndefined();
      const stored = await prisma.issuedDocument.findUniqueOrThrow({
        where: { id: mine.id },
      });
      expect(stored.emisParId).toBe(s.zola.id);
      const audit = await prisma.auditLog.findFirstOrThrow({
        where: { action: 'DOCUMENT_ISSUE', entiteId: mine.id },
      });
    ...
    it('un parent n’accède jamais à l’enfant d’une autre famille, ni à la carte', async () => {
      ...
      await portalPost(
        `/portal/children/${s.alice.id}/documents`,
        parent,
        CARTE,
      ).expect(404);
      await request(app.getHttpServer())
        .post(`/portal/children/${s.alice.id}/attestation`)
        .expect(401);
  ...
  describe('cartes d’une classe', () => {
    it('émet les cartes manquantes, sans dupliquer si on relance', async () => {
      ...
      const first = (
        await post(`/documents/class-cards/${s.classA.id}`).expect(201)
      ).body;
      ...
      expect(second.documents.map((d: { id: string }) => d.id).sort()).toEqual(
        first.documents.map((d: { id: string }) => d.id).sort(),
      );
      expect(await prisma.issuedDocument.count()).toBe(2);
    });
    ...
    it('refuse une classe inconnue, hors année active, et au-delà du plafond', async () => {
      ...
      const students = await prisma.student.findMany({
        where: { matricule: { startsWith: 'LOT-' } },
      });
      ...
      const res = await post(`/documents/class-cards/${s.classA.id}`).expect(
        422,
      );
  ...
  describe('photo d’élève', () => {
    ...
    it('est stockée hors du dossier public et servie seulement à un utilisateur connecté', async () => {
      ...
      await request(app.getHttpServer())
        .get(`/students/${s.alice.id}/photo`)
        .expect(401);
      // Jamais accessible par le dossier statique public.
      await request(app.getHttpServer())
        .get(`/uploads/${stored.photoUrl}`)
        .expect(404);
      await request(app.getHttpServer())
        .get(`/uploads/students/${stored.photoUrl}`)
        .expect(404);
      await request(app.getHttpServer())
        .get(`/private-uploads/students/${stored.photoUrl}`)
        .expect(404);
    });
    ...
    it('refuse un mauvais format ou un fichier trop lourd, sans rien enregistrer', async () => {
      ...
      expect(
        (await prisma.student.findUniqueOrThrow({ where: { id: s.alice.id } }))
          .photoUrl,
      ).toBeNull();
  ...
  describe('réglages et dossier', () => {
    it('la signature se téléverse en PNG ou JPEG, pas autrement, et par ceux qui gèrent les réglages', async () => {
      ...
      await upload(admin, Buffer.from('GIF89a'), 's.gif', 'image/gif').expect(
        400,
      );
      ...
      expect(res.body.signatureUrl).toMatch(
        /^\/uploads\/school\/signature-.+\.png$/,
      );
    ...
    it('le lieu de naissance est facultatif, modifiable et effaçable', async () => {
      const s = await school();
      const noPlace = (await s.mk('Sans', 'Lieu', '242060000009', undefined, {
        lieuNaissance: undefined,
      })) as { id: string };
      expect(
        (await get(`/students/${noPlace.id}`).expect(200)).body.lieuNaissance,
      ).toBeNull();
      expect(
        (await get(`/students/${s.alice.id}`).expect(200)).body.lieuNaissance,
      ).toBe('Pointe-Noire');
      await patch(`/students/${s.alice.id}`, admin, {
