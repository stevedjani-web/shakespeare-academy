...
describe('Vie scolaire, référentiel pédagogique (e2e, Lot 7)', () => {
  ...
  async function structure() {
    ...
    const level2 = (
      await post('/levels', { cycleId: cycle.id, code: 'CM1', nom: 'CM1' })
    ).body;
    const year = (
  ...
  describe('jours de classe (D52)', () => {
    it('vaut lundi à vendredi par défaut, et le samedi s’active par paramètre', async () => {
      ...
      const updated = await patch('/pedagogy/settings', {
        joursClasse: [6, 1, 2, 3, 4, 5],
      }).expect(200);
    ...
    it('journalise le changement (RG15)', async () => {
      ...
      const log = await prisma.auditLog.findFirst({
        where: { action: 'PEDAGOGY_SETTINGS_UPDATE' },
      });
  ...
  describe('créneaux horaires (D52, D53)', () => {
    it('crée un créneau commun sans rien de préchargé', async () => {
      ...
      const res = await post('/time-slots', {
        libelle: '1ère heure',
        heureDebut: '08:00',
        heureFin: '08:50',
      }).expect(201);
    ...
    it('refuse un chevauchement dans la même grille (409) mais accepte deux créneaux qui se suivent', async () => {
      ...
      const clash = await post('/time-slots', {
        libelle: 'H2',
        heureDebut: '08:30',
        heureFin: '09:20',
      }).expect(409);
  ...
  describe('salles', () => {
    it('crée, refuse un doublon insensible à la casse, désactive et supprime', async () => {
      const room = (
        await post('/rooms', { nom: 'Salle 1', capacite: 40 }).expect(201)
      ).body;
      await post('/rooms', { nom: 'salle  1' }).expect(409);
      const off = await patch(`/rooms/${room.id}`, { actif: false }).expect(
        200,
      );
  ...
  describe('matières et niveaux (D54)', () => {
    ...
    it('associe une matière à des niveaux et remplace l’ensemble', async () => {
      ...
      await put(`/subjects/${s.id}/levels`, {
        levels: [
          { levelId: level.id, minutesParSemaine: 240 },
          { levelId: level2.id },
        ],
      }).expect(200);
    ...
    it('refuse de supprimer une matière utilisée (409) et d’en retirer un niveau affecté', async () => {
      ...
      const t = (await post('/teachers', { nom: 'Ngoma', prenom: 'Paul' }))
        .body;
      ...
      const blocked = await put(`/subjects/${s.id}/levels`, {
        levels: [],
      }).expect(409);
  ...
  describe('enseignants (D56)', () => {
    ...
    it('enregistre les matières d’un enseignant et refuse une matière inconnue', async () => {
      ...
      await put(`/teachers/${t.id}/subjects`, {
    ...
    it('désactive un enseignant', async () => {
      const t = (await post('/teachers', { nom: 'Ngoma', prenom: 'Paul' }))
        .body;
      const res = await patch(`/teachers/${t.id}`, {
        statut: 'INACTIF',
      }).expect(200);
  ...
  describe('affectations enseignant × matière × classe', () => {
    async function setup() {
      ...
      const math = (await post('/subjects', { code: 'MATH', nom: 'Maths' }))
        .body;
      await put(`/subjects/${math.id}/levels`, {
        levels: [{ levelId: s.level.id }],
      });
      const teacher = (
        await post('/teachers', { nom: 'Ngoma', prenom: 'Paul' })
      ).body;
    ...
    it('refuse une matière non enseignée au niveau de la classe (422)', async () => {
      ...
      const other = (await post('/subjects', { code: 'ANG', nom: 'Anglais' }))
        .body;
    ...
    it('refuse un second enseignant sur la même classe et matière (RV01), mais permet de le remplacer', async () => {
      const { klass, math, teacher } = await setup();
      const other = (await post('/teachers', { nom: 'Ondze', prenom: 'Marie' }))
        .body;
      const a = (
        await post('/assignments', {
          classId: klass.id,
      ...
      const dup = await post('/assignments', {
        classId: klass.id,
        subjectId: math.id,
        teacherId: other.id,
      }).expect(409);
      ...
      const changed = await patch(`/assignments/${a.id}`, {
        teacherId: other.id,
      }).expect(200);
    ...
    it('filtre les affectations par enseignant', async () => {
      ...
      expect(
        (await get(`/assignments?teacherId=${teacher.id}`)).body,
      ).toHaveLength(1);
  ...
  describe('trimestres et calendrier (D55)', () => {
    it('crée trois trimestres numérotés automatiquement, sans chevauchement', async () => {
      ...
      expect((await get(`/terms?academicYearId=${year.id}`)).body).toHaveLength(
        2,
      );
    });
    ...
    it('enregistre vacances et jours fériés (un seul jour par défaut)', async () => {
      ...
      expect(
        (await get(`/calendar-events?academicYearId=${year.id}`)).body,
      ).toHaveLength(2);
    });
  ...
  describe('classe pilote et enseignant volontaire', () => {
    it('désigne une seule classe pilote à la fois et peut la retirer', async () => {
      ...
      const cleared = await put('/pedagogy/pilot', { classId: null }).expect(
        200,
      );
    ...
    it('désigne un seul enseignant volontaire, actif uniquement, sans toucher la classe si la clé est absente', async () => {
      ...
      const a = (await post('/teachers', { nom: 'Ngoma', prenom: 'Paul' }))
        .body;
      const b = (await post('/teachers', { nom: 'Ondze', prenom: 'Marie' }))
        .body;
      await put('/pedagogy/pilot', { classId: klass.id }).expect(200);
  ...
  describe('récapitulatif de saisie', () => {
    it('reflète ce qui est saisi et ce qui manque, pour l’année active', async () => {
      ...
      expect(s.affectations).toEqual({
        classes: 1,
        classesCompletes: 0,
        requises: 1,
        manquantes: 1,
      });
      expect(s.enseignants).toEqual({
        total: 2,
        actifs: 2,
        sansAffectation: 2,
      });
