        ...
        <Card>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id} className="border-b border-border align-top last:border-0 hover:bg-surface-muted">
                    <td className="whitespace-nowrap py-2.5 pr-4 text-ink-muted">
                      {new Date(log.createdAt).toLocaleString("fr-FR")}
                    </td>
                    <td className="py-2.5 pr-4 text-ink">{log.user ? `${log.user.prenom} ${log.user.nom}` : "—"}</td>
